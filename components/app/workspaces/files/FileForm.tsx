...
export default function FileForm(props: FileFormProps) {
  ...
  const uploadHook = useWorkspaceFileUploadMutationHook({
    onSuccess(data, params) {
      ...
      // if (folderpath) filepath += folderpath + folderConstants.nameSeparator;
      // filepath += data.body.file.name;
      // if (data.body.file.extension)
      //   filepath +=
      //     fileConstants.fileNameAndExtensionSeparator +
      //     data.body.file.extension;

      // filepath = addRootnameToPath(filepath, workspaceRootname);
      // progressHandlerHook.mergeProgress(filepath, {
      //   loaded: data.body.file.size,
      //   progress: 100,
      // });
  ...
  const submitFile = async (input: SingleFileFormValue) => {
    ...
    if (input.resourceId) {
      ...
      if (input.file) {
        // progressHandlerHook.setProgress(filepath, {
